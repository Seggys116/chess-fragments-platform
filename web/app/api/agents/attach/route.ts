...
export async function POST(req: NextRequest) {
    try {
        ...
        // Rate limit: 10 agent attach/create operations per minute per user
        const rateLimitResult = await userRateLimit(user.id, 'create');
        if (!rateLimitResult.allowed) {
            return NextResponse.json(
                { error: 'Too many agent operations. Please wait a moment and try again.' },
                { status: 429 }
            );
        }

        const { agentId, name, version } = await req.json();

        let agent;
        let isNewAgent = false;

        if (agentId) {
            // Linking existing agent
            ...
            if (agent.validationStatus !== 'passed') {
                return NextResponse.json({ error: 'Agent must pass validation before attaching' }, { status: 400 });
            }

            // Ensure ranking exists for existing agent
            if (!agent.ranking) {
        ...
        } else if (name && version !== undefined) {
            // Creating new local agent (no code validation required)
            ...
            if (existing) {
                return NextResponse.json({
                    error: 'Agent with this name and version already exists. Choose a different version or update the existing agent.'
                }, { status: 400 });
            }
            ...
            agent = await prisma.agent.create({
                data: {
                    ...
                    name: name,
                    version: versionInt,
                    codeText: '# Local agent - code not stored on server',
                    codeHash: crypto.createHash('sha256').update('local').digest('hex'),
                    executionMode: 'local',
                    validationStatus: 'passed', // Local agents don't need validation
                    active: true,
                    ranking: {
                        create: {
                            eloRating: 1500,
            ...
            return NextResponse.json({
                error: 'Either agentId or both name and version must be provided'
            }, { status: 400 });
        }

        // Generate secure connection token using HMAC
        ...
        // Store the token hash (not the actual token) in the database
        ...
        const pythonScript = generateClientScript(agent.id, connectionToken, agent.name);
        ...
        return NextResponse.json({
            ...
            script: pythonScript,
            message: 'Save this script as agent_connector.py in the same directory as your agent.py',
        });
    } catch (error) {
        console.error('Error creating local agent attachment:', error);
        return NextResponse.json(
