...
function isUploadBlackoutDay(): boolean {
    const now = new Date();
    // Block uploads every 12 December (UTC/GMT) for tournament freeze
    return now.getUTCMonth() === 11 && now.getUTCDate() === 12;
}
...
function validateAgentCode(code: string): { valid: boolean; error?: string } {
    ...
    const forbiddenImports = ['os', 'subprocess', 'socket', 'urllib', 'requests', 'sys', 'multiprocessing', 'ctypes', 'importlib', 'builtins', '__import__'];
    ...
    // Check for dangerous built-in function calls (eval/exec don't need imports)
    // Remove comments and string literals before checking to avoid false positives
    ...
    const forbiddenBuiltins = ['eval', 'exec', 'compile', '__import__'];
    for (const forbidden of forbiddenBuiltins) {
        // Match function calls like eval(, exec(, etc. with word boundary
        const pattern = new RegExp(`\\b${forbidden}\\s*\\(`);
        if (pattern.test(codeWithoutComments)) {
            return { valid: false, error: `Forbidden function: ${forbidden}()` };
        }
    }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (isUploadBlackoutDay()) {
            return NextResponse.json(
                { error: 'Agent uploads are paused on 12 December (GMT) for tournament operations.' },
                { status: 403 }
            );
        }
